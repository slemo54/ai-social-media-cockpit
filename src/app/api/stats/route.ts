    ...
    const stats = {
      ...
      trends: {
        last30Days: {
          ...
          published: last30Days.filter((p: any) => p.status === 'published').length,
          words: last30Days.reduce((sum: number, p: any) => sum + (p.word_count || 0), 0),
        },
        last7Days: {
          total: last7Days.length,
          published: last7Days.filter((p: any) => p.status === 'published').length,
          words: last7Days.reduce((sum: number, p: any) => sum + (p.word_count || 0), 0),
        },
...
function calculateGrowthRate(posts: any[], sevenDaysAgo: Date) {
  const recent = posts.filter((p: any) => new Date(p.created_at) >= sevenDaysAgo).length;
  const previous = posts.filter((p: any) => {
